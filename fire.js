...
function createFireStructure() {
    const numberOfPixels = canvasHeight * canvasWidth
...
function createFireSource() {
    const floor = (canvasHeight * canvasWidth) - canvasWidth;
...
function updateFireIntensity(index) {
    ...
    if (bellow >= canvasHeight * canvasWidth ) return

    const decay = Math.floor(Math.random() * 3) 

    const newIntensity = firePixelsData[bellow] - decay

    const newIndex = index - decay
